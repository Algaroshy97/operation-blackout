...
// silently drop skinned ones. Paint a soldier into an offscreen corner and
// verify pixels changed; if not, fall back to the simple enemy mesh everywhere.
function probeSkinnedSoldier() {
  if (!GLB_PARSED.SOLDIER) return;
  try {
    const probe = spawnEnemy(0, 0, -35);      // clear strip behind the north barricade
    ...
    const v = new THREE.Vector3(0, 1.0, -60).project(camera);
    ...
    for (let i = 0; i < 9; i++) {
      ...
      // sky (0x8aa4c8) / fog counts as NOT painted; anything else does
      if (!(Math.abs(r - 138) < 30 && Math.abs(g - 164) < 30 && Math.abs(b - 200) < 40)) painted++;
    }
    // remove the probe enemy cleanly (no score/kill side effects)
    probe.dead = true; probe.deathT = 99; updateEnemies(0);
    ...
    // reset camera to player pose so no frame renders from the probe position
    player.pos.set(0, CFG.player.height, 24);
    player.yaw = Math.PI; player.pitch = 0;
  } catch (err) {
    GLB_SOLDIER_BROKEN = true;   // probe failure = cannot trust skinned path
...
function spawnEnemy(kind, x, z) {
  ...
  // Kenney mini-soldier GLB raw height is ~0.84 m — must be scaled to human height
  // (hitboxes assume ~1.85 m). Scale the ROOT (skinned model only); hitboxes stay
  // OUTSIDE the scaled root so their world sizes never depend on the model scale.
  const GLB_SOLDIER_SCALE = 1.85 / 0.84;   // ≈ 2.2
  if (GLB_PARSED.SOLDIER && !mobileSafe && !GLB_SOLDIER_BROKEN) {
    ...
    // skinned bounds can desync from the animated pose on some GPUs -> never cull
    root.traverse(function (o) { if (o.isSkinnedMesh) o.frustumCulled = false; });
    // procedural hitboxes for consistent aim behavior (outside the scaled root)
    const hbMat = new THREE.MeshBasicMaterial({ visible: false });
    const hitBody = new THREE.Mesh(new THREE.BoxGeometry(0.62, 1.05, 0.5), hbMat);
