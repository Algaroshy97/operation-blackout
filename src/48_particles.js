...
// regardless of particle count. Additive colours go well above 1.0 on purpose:
// that headroom is what the post-FX bloom picks up, and the direct path tone
// maps it like any other material.
const PFX_MAX = 3000;
...
function makePfxLayer(additive, tex) {
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: [
      ...
      'uniform float scale, maxPt, fogNear, fogFar;',
...
function flashLight(pos, color, intensity, distance, life) {
  ...
  best.userData.life = life; best.userData.max = life; best.userData.peak = intensity * LIGHT_COMPAT;
  best.intensity = intensity * LIGHT_COMPAT;
}
...
// ---- Effect library ----
function pfxRnd(a, b) { return a + Math.random() * (b - a); }
function fxMuzzle(p, d, big) {
  const n = big ? 5 : 2;
  for (let i = 0; i < n; i++) {
    const s = pfxRnd(0.4, 1.2);
    pfxEmit(PFX_SMOKE, p.x, p.y, p.z, d.x * s + pfxRnd(-0.2, 0.2), d.y * s + pfxRnd(0.1, 0.4), d.z * s + pfxRnd(-0.2, 0.2),
      pfxRnd(0.5, 1.0), 0.08, big ? 0.9 : 0.5, 0x9a948c, 0x6a6660, 0.22, 2.2, -0.5, 0);
  }
  for (let i = 0; i < (big ? 6 : 3); i++) {
    const s = pfxRnd(6, 14);
    pfxEmit(PFX_ADD, p.x, p.y, p.z, d.x * s + pfxRnd(-1, 1), d.y * s + pfxRnd(-1, 1), d.z * s + pfxRnd(-1, 1),
      pfxRnd(0.04, 0.1), 0.03, 0.01, 0xffd890, 0xff7020, 2.5, 4, 0, 0);
  }
}
// Effect class for a raycast hit object. Brick and the ground get their own
// dust colours; everything else follows the penetration class, and a hit with no
// object (a shield plate) throws metal sparks.
function fxSurfaceFor(obj) {
  if (!obj) return 'metal';
  if (obj === ground) return 'ground';
  const mat = Array.isArray(obj.material) ? obj.material[0] : obj.material;
  if (mat === MAT.brick) return 'brick';
  return penMaterialFor(mat);
}
const SURFACE_FX = {
...
function fxImpact(p, n, surface) {
  ...
  for (let i = 0; i < 3; i++) {
    const s = pfxRnd(0.6, 1.8);
    pfxEmit(PFX_SMOKE, p.x + nx * 0.05, p.y + ny * 0.05, p.z + nz * 0.05,
      nx * s + pfxRnd(-0.4, 0.4), ny * s + pfxRnd(0, 0.5), nz * s + pfxRnd(-0.4, 0.4),
      pfxRnd(0.6, 1.3), 0.1, pfxRnd(0.5, 0.9), S.dust, S.dust2, 0.55, 3, -0.2, 0);
  }
  // chips / splinters (small, heavy, bouncing)
  if (S.chips) for (let i = 0; i < 5; i++) {
    const s = pfxRnd(2, 4.5);
    pfxEmit(PFX_SMOKE, p.x, p.y, p.z, nx * s + pfxRnd(-1.5, 1.5), ny * s + pfxRnd(0.5, 2.5), nz * s + pfxRnd(-1.5, 1.5),
      pfxRnd(0.5, 0.9), 0.035, 0.03, S.chips, S.chips, 1.0, 0.5, 11, PF_BOUNCE);
  }
  // sparks: bright, fast, gravity-bound, bounce on the ground
  for (let i = 0; i < S.sparks; i++) {
    const s = pfxRnd(3, 8);
    pfxEmit(PFX_ADD, p.x, p.y, p.z, nx * s + pfxRnd(-3, 3), ny * s + pfxRnd(0, 3), nz * s + pfxRnd(-3, 3),
      pfxRnd(0.15, 0.4), 0.035, 0.015, 0xfff0b0, 0xff6010, 3, 1.5, 12, PF_BOUNCE);
  }
  if (S.sparks > 2) flashLight(p, 0xffb060, 1.2, 4, 0.06);
}
function fxBlood(p, dir, isHead) {
  const n = isHead ? 10 : 6;
  for (let i = 0; i < n; i++) {
    const s = pfxRnd(1, 3.5);
    pfxEmit(PFX_SMOKE, p.x, p.y, p.z, dir.x * s + pfxRnd(-1, 1), dir.y * s + pfxRnd(-0.2, 1.4), dir.z * s + pfxRnd(-1, 1),
      pfxRnd(0.4, 0.8), 0.06, 0.04, 0x5a0404, 0x2a0202, 1.0, 1, 12, PF_STICK);
  }
  for (let i = 0; i < (isHead ? 4 : 2); i++) {
    pfxEmit(PFX_SMOKE, p.x, p.y, p.z, dir.x * 0.8 + pfxRnd(-0.3, 0.3), pfxRnd(0, 0.4), dir.z * 0.8 + pfxRnd(-0.3, 0.3),
...
function fxDust(p, n, spread, color) {
  for (let i = 0; i < n; i++) {
    ...
    pfxEmit(PFX_SMOKE, p.x + Math.cos(a) * 0.3, 0.12, p.z + Math.sin(a) * 0.3, Math.cos(a) * s, pfxRnd(0.2, 0.7), Math.sin(a) * s,
      pfxRnd(0.6, 1.2), 0.25, pfxRnd(0.8, 1.4), color || 0x8a8076, 0x5a544e, 0.4, 2.5, -0.1, 0);
  }
}
function fxExplosion(p, scale) {
  ...
  for (let i = 0; i < 18 * scale; i++) {
    const a = Math.random() * 6.283, b = Math.random() * 3.1416, s = pfxRnd(2, 7) * scale;
    pfxEmit(PFX_ADD, p.x, p.y + 0.3, p.z, Math.cos(a) * Math.sin(b) * s, Math.abs(Math.cos(b)) * s * 0.9 + 1, Math.sin(a) * Math.sin(b) * s,
      pfxRnd(0.25, 0.55), pfxRnd(0.8, 1.4) * scale, pfxRnd(2.2, 3.4) * scale, 0xfff0c0, 0xff3808, 2.6, 5, -2, 0);
  }
  // sparks / embers
  for (let i = 0; i < 40 * scale; i++) {
    const a = Math.random() * 6.283, s = pfxRnd(5, 16);
    pfxEmit(PFX_ADD, p.x, p.y + 0.3, p.z, Math.cos(a) * s, pfxRnd(3, 12), Math.sin(a) * s,
      pfxRnd(0.6, 1.6), 0.07, 0.03, 0xffe0a0, 0xff4010, 3, 1.2, 14, PF_BOUNCE);
  }
  // rolling black smoke column
  for (let i = 0; i < 22 * scale; i++) {
    const a = Math.random() * 6.283, r = pfxRnd(0, 1.5) * scale;
    pfxEmit(PFX_SMOKE, p.x + Math.cos(a) * r, p.y + pfxRnd(0.3, 1.6), p.z + Math.sin(a) * r,
      Math.cos(a) * pfxRnd(0.5, 2.5), pfxRnd(1.2, 3.5), Math.sin(a) * pfxRnd(0.5, 2.5),
      pfxRnd(2.5, 4.5), pfxRnd(1.0, 1.8) * scale, pfxRnd(4, 6.5) * scale, 0x3a3230, 0x1c1a1a, 0.75, 0.9, -0.35, 0);
  }
  // ground dust ring
  for (let i = 0; i < 16 * scale; i++) {
    const a = i / (16 * scale) * 6.283, s = pfxRnd(6, 10);
    pfxEmit(PFX_SMOKE, p.x, 0.25, p.z, Math.cos(a) * s, pfxRnd(0.2, 0.8), Math.sin(a) * s,
      pfxRnd(1.2, 2.0), 0.5, pfxRnd(2, 3), 0x7a6e62, 0x4a443e, 0.5, 2.2, 0, 0);
  }
  // debris chunks
  for (let i = 0; i < 14 * scale; i++) {
    const a = Math.random() * 6.283, s = pfxRnd(4, 10);
    pfxEmit(PFX_SMOKE, p.x, p.y + 0.3, p.z, Math.cos(a) * s, pfxRnd(4, 11), Math.sin(a) * s,
...
function sniperTrail(from, to) {
  ...
  for (let i = 1; i <= n; i++) {
    const t = i / (n + 1);
    pfxEmit(PFX_SMOKE, from.x + dx * t, from.y + dy * t, from.z + dz * t, pfxRnd(-0.08, 0.08), pfxRnd(0.05, 0.2), pfxRnd(-0.08, 0.08),
      pfxRnd(1.2, 2.2), 0.05, 0.35 + t * 0.2, 0xc8ccd0, 0x8a8e94, 0.16, 1.2, -0.05, 0);
  }
}
...
function fxFire(p, strength, dt) {
  ...
  while (n-- > 0) {
    pfxEmit(PFX_ADD, p.x + pfxRnd(-0.22, 0.22) * strength, p.y, p.z + pfxRnd(-0.22, 0.22) * strength,
      pfxRnd(-0.3, 0.3), pfxRnd(1.4, 2.6) * (0.6 + strength * 0.4), pfxRnd(-0.3, 0.3),
      pfxRnd(0.35, 0.7), pfxRnd(0.35, 0.6) * strength, 0.06, 0xffc070, 0xc01804, 2.4, 0.8, -1.2, 0);
    if (Math.random() < 0.35) pfxEmit(PFX_SMOKE, p.x + pfxRnd(-0.2, 0.2), p.y + 0.8 * strength, p.z + pfxRnd(-0.2, 0.2),
      pfxRnd(-0.2, 0.2), pfxRnd(1.0, 1.8), pfxRnd(-0.2, 0.2), pfxRnd(1.8, 3.2), 0.4 * strength, pfxRnd(1.6, 2.6) * strength, 0x2e2a28, 0x1a1818, 0.45, 0.4, -0.4, 0);
    if (Math.random() < 0.08) pfxEmit(PFX_ADD, p.x, p.y + 0.3, p.z, pfxRnd(-0.8, 0.8), pfxRnd(2, 4), pfxRnd(-0.8, 0.8),
      pfxRnd(0.8, 1.5), 0.04, 0.02, 0xffd080, 0xff5010, 3, 0.4, 1.5, 0);
  }
}
