...
// the look identical between runs.
...
function makeSurface(size, seed, normalStrength, draw) {
  ...
  return { map: toTex(col.c, true), normalMap: toTex(heightToNormal(hgt.c, normalStrength), false) };
}
...
TEX.asphalt = makeSurface(512, 53, 2.5, function (g, h, S, rnd) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    g.fillStyle = 'rgba(28,29,33,0.55)'; g.fillRect(x, y, w, hh);
    g.strokeStyle = 'rgba(20,20,22,0.7)'; g.lineWidth = 2; g.strokeRect(x, y, w, hh);
  }
...
TEX.barrel = (function () {
  ...
  return { map: toTex(col.c, true), normalMap: toTex(heightToNormal(hgt.c, 3), false) };
})();
